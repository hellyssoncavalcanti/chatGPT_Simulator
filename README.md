...
- **`Scripts/browser.py`**
  ...
  - **Round-robin de perfis**: `_choose_profile_for_new_chat` trata `browser_profile="default"` (ou omitido) como "sem preferência explícita", alternando automaticamente entre `default` e `segunda_chance`. Antes, qualquer string truthy (inclusive `"default"`) cortocircuitava o round-robin.
  - **Failover bilateral de rate-limit**: ao atingir o rate-limit, o worker troca automaticamente de perfil em qualquer direção (`default`→`segunda_chance` ou `segunda_chance`→`default`). Proteção anti-loop por `tried_profiles` — cada perfil é tentado no máximo uma vez por tarefa; esgotados todos os elegíveis, o erro é propagado ao cliente.
...
Cada chat persistido guarda `chromium_profile` (nome da pasta do perfil Chromium usado na execução, ex.: `chrome_profile`, `chrome_profile_segunda_chance`).
...
**Perfis disponíveis (`config.CHROMIUM_PROFILES`):**

| Chave | Diretório | Uso |
|---|---|---|
| `default` | `chrome_profile/` | Perfil principal — conta ChatGPT Plus do operador |
| `segunda_chance` | `chrome_profile_segunda_chance/` | Perfil alternativo — round-robin e failover |
| `analisador` | `chrome_profile_analisador/` | Reservado ao pipeline clínico |

**Fluxo de seleção de perfil:**
1. Se `browser_profile` for omitido ou `null`/`"default"` → round-robin automático entre `default` e `segunda_chance`.
2. Se `browser_profile` for um nome explícito válido → perfil fixado para a requisição.
3. Rate-limit em qualquer perfil → failover bilateral imediato para o outro (`default`↔`segunda_chance`), com proteção anti-loop via `tried_profiles` (cada perfil tentado no máximo uma vez por tarefa).
4. Chave inexistente em `config.CHROMIUM_PROFILES` → cai em `default`.
...
3. Conta `N = len(config.CHROMIUM_PROFILES)` (atualmente 3: `default` + `segunda_chance` + `analisador`).
